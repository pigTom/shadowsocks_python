...
class SpeedTester(object):
    ...
    网速测试：
    比如最大网速是1kByte/s
    就是在1秒钟内传输的数据大是 1024 Byte
    """
    def __init__(self, max_speed = 0):
        # max_speed 最大速度，如果max_speed = 0, 则不限速
...
class TCPRelayHandler(object):
    def __init__(self, server, fd_to_handlers, loop, local_sock, config,
        ...
        # 将处理器加入到fd_to_handlers中
        # fd_to_handler是处理器集合
        fd_to_handlers[self._local_sock_fd] = self
        # 将新建的连接加入到事件循环中,但是处理器是TCPRelay
        # 意思是TCPRelay也是事件分发器，处理自己的新建连接和socket的TCP读事件？
        # TCP的写事件呢
    ...
    def _update_stream(self, stream, status):
        ...
        if dirty:
            if self._local_sock:
                ...
                # downstream writing local(socket5 client)
                if self._downstream_status & WAIT_STATUS_WRITING:
                    event |= eventloop.POLL_OUT

                # upstream reading from local(socket5 client)
            ...
            if self._remote_sock:
                ...
                # downstream reading remote
                if self._downstream_status & WAIT_STATUS_READING:
                    event |= eventloop.POLL_IN

                # upstream writing remote
    ...
    def _handle_stage_addr(self, ogn_data, data):
        """
        如果_is_local为真，且_stage == STAGE_ADDR
        处理地址阶段
        :param ogn_data: 原始数据
        :param data: 原始数据或加密过后的数据（用来进行传输)
        :return:
        """
        try:
            if self._is_local:
                # +-----+-----+-----+------+----------+----------+
                # | VER | CMD | RSV | ATYP | AST.ADDR | DST.PORT |
                # +-----+-----+-----+------+----------+----------+
                # |  1  |  1  |0X00 |  1   | Variable |    	2    |
                # +-----+-----+-----+------+----------+----------+
                # 如果cmd是UDP Associate会怎么样
                cmd = common.ord(data[1])
                # 如果是UDP ASSOCIATE，需要将地址信息返回？
                if cmd == CMD_UDP_ASSOCIATE:
                    ...
                    addr_to_send = socket.inet_pton(self._local_sock.family,
                                                    addr)
                    # >H for big endian order, H for unsigned short, 2 bytes
                    #
            ...
            if self._is_local:
                ...
                # echo b'\x05\x00\x00\x01' and b'\x00\x00\x00\x00\x10\x10' to client
                # as address reply of require
                # ip address: 0.0.0.0, port (4096+16)
                ...
                # self._encryptor不为空（在初始化时被赋值，除非self.destroy()
                if self._encryptor is not None:
                    data = self._protocol.client_pre_encrypt(data)
                    data_to_send = self._encryptor.encrypt(data)
                    data_to_send = self._obfs.client_encode(data_to_send)
                # 所以data_to_send不为空，除非它已经destroy了
                else:
                    return
    ...
    def _handle_dns_resolved(self, result, error):
        """nds resoleve之后会新建remote_sock，连接remote的socket"""
        if error:
    ...
    def handle_event(self, sock, fd, event):
        ...
        if self._user is not None and self._user not in self._server.server_users:
            self.destroy()
            return True
        # 通过fd是否_remote_sock_fd或_remotev6_sock_fd判断sock的类型
    ...
    def destroy(self):
        ...
        if self._remote_address:
            ...
            logging.debug('destroy: %s:%d' %(addr, port))
        else:
...
class TCPRelay(object):
    def __init__(self, config, dns_resolver, is_local, stat_callback=None, stat_counter=None):
        ...
        if is_local:
            # 监听的地址， socket用它来绑定监听的地址
            listen_addr = config['local_address']
            # 监听的端口，socket用它来绑定监听的端口
        ...
        # 新建一个socket, 指定address family:AddressFamily.AF_INET6: 30
    ...
    def handle_event(self, sock, fd, event):
        ...
        if sock == self._server_socket:
            ...
            try:
                ...
                # 生成一个新的socket，将新的socket封装到TCPRelayHandler中
                # 但是生成的Handler有加入EventLoop中吗，如何加入的
                # 传过去的_fd_to_handlers加入了
                ...
                if error_no in (errno.EAGAIN, errno.EINPROGRESS,
                    ...
                    if handler:
                        handler.destroy()
        # 由于TCPRelayHandler将socket绑定到TCPRelay上了
        # 所以 sock为 conn = self._server_socket.accept()
        else:
            if sock:
